					...
					"<table class='table table-striped table-hover'>" +
						"<thead>" +
							...
							"<!-- ko foreach: columns-->" +
								...
								"<!-- /ko -->" +
								"<!-- ko if: sortable-->" +
								"<th data-bind='class : { className: spanWidth, isCursorPointer: sortable }, click: $parent.sort.bind($data, $parent)'> " +
	...
	function Grid(tableConfig) {
		...
		var templateHtml = {
			...
			'ExpandableHeader': "<h2 class='expanded cursorPointer' data-bind='text: caption, " +
															   "click: toggleExpandCollapseContainer.bind($data, isExpanded), " +
		...
		var getTableHtml = function (config) {
		...
		var getFormattedCellHtml = function (column) {
		...
		var getColumnHtml = function (columns) {
			...
			if (!!columns && columns.length > 0) {
				...
				for (var index = 0; index < noOfColumns; index++) {
					...
					if (column.hasOwnProperty('formatter')) {
						...
						getFormattedCellHtml(column.id);
					}
		...
		var _placeHolder = {
			'columns': getColumnHtml
		};

		//#endregion

		// Create customized templates if configuration not provided by user
		getTableHtml(grid.tableObject);
		getColumnHtml(grid.tableObject.columns);

		//#region Functions

		/// <summary> Update sort column on click </summary> 
		grid.tableObject.sort = function (tableObject, column) {
		...
		grid.tableObject.toggleExpandCollapseContainer = function (value) {
